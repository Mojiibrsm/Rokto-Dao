...
import { 
  Phone, MapPin, ShieldCheck, 
  ArrowLeft, Share2, Users, Info, MessageSquare, Loader2, ShieldAlert, AlertTriangle, Trophy
} from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Card, CardContent, CardHeader, CardTitle, CardDescription } from '@/components/ui/card';
import { Badge } from '@/components/ui/badge';
import { getDonorBySlug, getDonorByPhone, sendMessage, submitReport, type Donor } from '@/lib/sheets';
...
export default function DonorProfilePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  useEffect(() => {
    ...
    async function loadDonor() {
      try {
        // Try fetching by slug first, fallback to phone for backward compatibility
        let data = await getDonorBySlug(slug);
        if (!data && slug.match(/^\d+$/)) {
          data = await getDonorByPhone(slug);
        }
        setDonor(data);
      } catch (e) {
        console.error(e);
  ...
  const handleStartChat = async () => {
    ...
    setIsMessaging(true);
    try {
      const res = await sendMessage(user.phone, donor!.phone, "আসসালামু আলাইকুম, আমি আপনার সাথে যোগাযোগ করতে চাই।");
      if (res.success) {
