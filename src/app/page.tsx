...
export default function Home() {
  ...
  const handleShare = async (req: BloodRequest) => {
    ...
    try {
      ...
      toast({ title: lang === 'bn' ? "কপি হয়েছে!" : "Copied!" });
    } catch (err) {
      toast({ variant: "destructive", title: lang === 'bn' ? "ব্যর্থ হয়েছে" : "Failed" });
    }
  };
      ...
      <section className="relative w-full py-20 md:py-32 flex flex-col items-center justify-center bg-accent/30 text-center px-4 overflow-hidden border-b-4 border-primary/10">
        ...
        <div className="container mx-auto relative z-10 max-w-5xl space-y-10">
          <div className="inline-flex items-center gap-2 bg-white border-2 border-primary/20 px-6 py-2 rounded-full shadow-md animate-bounce">
            ...
            <span className="text-primary font-black uppercase tracking-widest text-[12px]">{t.home.heroBadge}</span>
          </div>
          <div className="space-y-6">
            <h1 className="text-5xl md:text-8xl font-black tracking-tight text-primary font-headline leading-tight drop-shadow-sm">
              {t.home.heroTitlePrefix} <br /><span className="text-foreground">{t.home.heroTitleSuffix}</span>
            </h1>
            ...
            <div className="bg-white p-4 rounded-[2.5rem] shadow-2xl border-4 border-primary/10 flex flex-col md:row gap-4">
              <div className="flex flex-col md:flex-row flex-1 divide-y md:divide-y-0 md:divide-x-2 divide-primary/5">
                <div className="flex-1 px-6 py-3">
                  <label className="block text-[11px] font-black text-primary uppercase text-left mb-2 tracking-wider">{t.common.bloodGroup}</label>
                  <Select value={selectedBloodType} onValueChange={setSelectedBloodType}>
                    <SelectTrigger className="h-12 border-none bg-accent/50 rounded-xl focus:ring-2 focus:ring-primary text-lg font-black text-foreground px-4">
                      <div className="flex items-center gap-3">
                        ...
                        <SelectValue placeholder={t.common.bloodGroup} />
                      </div>
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="যেকোনো গ্রুপ" className="font-bold">{lang === 'bn' ? 'যেকোনো গ্রুপ' : 'Any Group'}</SelectItem>
                ...
                <div className="flex-1 px-6 py-3">
                  <label className="block text-[11px] font-black text-primary uppercase text-left mb-2 tracking-wider">{t.common.district}</label>
                  <Select value={selectedDistrict} onValueChange={setSelectedDistrict}>
                    ...
                    <SelectContent>
                      <SelectItem value="যেকোনো জেলা" className="font-bold">{lang === 'bn' ? 'যেকোনো জেলা' : 'Any District'}</SelectItem>
                      {DISTRICTS.map(d => (
              ...
              <Button onClick={handleSearch} className="h-16 md:h-auto px-12 bg-primary hover:bg-primary/90 text-white rounded-2xl shadow-xl shadow-primary/30 transition-all font-black text-2xl gap-3">
                <Search className="h-7 w-7" /> {t.common.search}
              </Button>
            </div>
          </div>
        </div>
      ...
      <section className="bg-white py-16 border-b-2 border-accent">
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-10">
            {[
              { val: "২৫,০০০+", label: t.home.statsDonors, icon: Users, color: "text-blue-600", bg: "bg-blue-50" },
              { val: "১৫,০০০+", label: t.home.statsRequests, icon: Droplet, color: "text-primary", bg: "bg-primary/5" },
              { val: "১২,৫০০+", label: t.home.statsSuccess, icon: Heart, color: "text-red-600", bg: "bg-red-50" },
              { val: "৬৪", label: t.home.statsDistricts, icon: Globe, color: "text-green-600", bg: "bg-green-50" }
            ].map((stat, i) => (
      ...
      <section className="py-24 bg-white">
        <div className="container mx-auto px-4">
          <div className="text-center mb-16 space-y-4">
            <h2 className="text-4xl md:text-6xl font-black font-headline">{lang === 'bn' ? 'আমাদের' : 'Our'} <span className="text-primary">{t.home.donorsTitle}</span></h2>
            <p className="text-xl text-muted-foreground font-bold italic opacity-70">
              {lang === 'bn' ? '"বাংলাদেশের শ্রেষ্ঠ স্বেচ্ছাসেবী রক্তদাতাদের তালিকা"' : '"List of the best voluntary blood donors in Bangladesh"'}
            </p>
                  ...
                  <Card key={idx} className="overflow-hidden border-2 border-primary/5 shadow-xl hover:shadow-primary/10 transition-all rounded-[2.5rem] group bg-accent/5 flex flex-col">
                    ...
                    <CardContent className="pt-8 px-8 space-y-6 flex-grow">
                      <div className="grid grid-cols-2 gap-4">
                        <div className="p-5 bg-white rounded-3xl border-2 border-primary/5 shadow-sm text-center">
                          <p className="text-primary uppercase text-[10px] font-black mb-2 tracking-widest">{lang === 'bn' ? 'শেষ রক্তদান' : 'Last Donation'}</p>
                          <p className="font-black text-lg text-foreground">{donor.lastDonationDate || 'N/A'}</p>
                        </div>
                        <div className="p-5 bg-white rounded-3xl border-2 border-primary/5 shadow-sm text-center">
                          <p className="text-primary uppercase text-[10px] font-black mb-2 tracking-widest">{lang === 'bn' ? 'মোট রক্তদান' : 'Total Donations'}</p>
                          <p className="font-black text-3xl text-primary">{donor.totalDonations} {lang === 'bn' ? 'বার' : 'Times'}</p>
                        </div>
                      </div>
                    </CardContent>
                    <CardFooter className="p-0 mt-4 flex">
                      <Button className="flex-1 h-16 rounded-none bg-primary hover:bg-secondary text-xl font-black gap-4 transition-all" asChild>
                        <a href={`tel:${donor.phone}`}><Phone className="h-6 w-6" /> {t.common.call}</a>
                      </Button>
                      <Button variant="ghost" className="flex-1 h-16 rounded-none text-primary font-black text-xl" asChild>
                        <NextLink href={`/donors/${donor.phone}`}>{t.common.profile} <ExternalLink className="h-5 w-5 ml-2" /></NextLink>
                      </Button>
                    </CardFooter>
                  </Card>
                );
          ...
          <div className="text-center">
            <Button size="lg" className="rounded-full px-16 h-16 text-2xl bg-white border-4 border-primary text-primary hover:bg-primary hover:text-white font-black transition-all shadow-xl shadow-primary/10" asChild>
              <NextLink href="/donors">{lang === 'bn' ? 'সব রক্তদাতা তালিকা দেখুন' : 'View All Donor List'} <ArrowRight className="ml-3 h-7 w-7" /></NextLink>
            </Button>
          </div>
        </div>
      </section>
        ...
        <div className="container mx-auto px-4">
          <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-8 text-center md:text-left">
            <div className="space-y-4">
              ...
              <p className="text-xl text-muted-foreground font-bold">{lang === 'bn' ? 'এখনই যাদের রক্ত প্রয়োজন — একটি শেয়ার বাঁচাতে পারে একটি প্রাণ।' : 'Those who need blood now — one share can save a life.'}</p>
            </div>
            <Button size="lg" className="bg-primary hover:bg-secondary text-white rounded-full px-10 h-14 text-xl font-black shadow-2xl shadow-primary/30 gap-3" asChild>
              <NextLink href="/requests">{t.common.viewAll} <ArrowRight className="h-6 w-6" /></NextLink>
            </Button>
          </div>
          {loadingRequests ? (
            <div className="flex justify-center py-20"><Loader2 className="animate-spin h-16 w-16 text-primary" /></div>
                ...
                <Card key={req.id} className="overflow-hidden border-4 border-white shadow-[0_20px_50px_rgba(0,0,0,0.1)] hover:shadow-primary/20 transition-all rounded-[3.5rem] bg-white group">
                  ...
                  <CardHeader className="p-8 pb-4">
                    <div className="flex justify-between items-start gap-4">
                      <div className="space-y-2">
                        <CardTitle className="text-3xl font-black group-hover:text-primary transition-colors">{req.patientName || (lang === 'bn' ? 'নাম প্রকাশে অনিচ্ছুক' : 'Anonymous')}</CardTitle>
                      ...
                      <Badge className={`${req.isUrgent ? 'bg-primary' : 'bg-slate-800'} text-white text-sm uppercase font-black py-2 px-6 h-10 rounded-full shadow-lg`}>
                        {req.isUrgent ? (lang === 'bn' ? 'জরুরি' : 'Urgent') : 'Approved'}
                      </Badge>
                    </div>
                  </CardHeader>
                  <CardContent className="px-8 pb-10">
                    <div className="grid grid-cols-3 gap-6 py-8 border-y-4 border-dashed border-accent my-6">
                      <div className="text-center">
                        <p className="text-[11px] uppercase font-black text-muted-foreground mb-2 tracking-[0.2em]">{t.common.bloodGroup}</p>
                      ...
                      <div className="text-center border-x-4 border-accent px-4">
                        <p className="text-[11px] uppercase font-black text-muted-foreground mb-2 tracking-[0.2em]">{lang === 'bn' ? 'পরিমাণ' : 'Amount'}</p>
                        <p className="text-5xl font-black text-foreground">{req.bagsNeeded}</p>
                        <p className="text-[10px] font-black text-muted-foreground uppercase">{lang === 'bn' ? 'ব্যাগ' : 'Bags'}</p>
                      </div>
                      <div className="text-center">
                        <p className="text-[11px] uppercase font-black text-muted-foreground mb-2 tracking-[0.2em]">{lang === 'bn' ? 'প্রয়োজন' : 'Needed'}</p>
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 mt-8">
                      <Button className="flex-1 bg-primary hover:bg-secondary rounded-[2rem] h-16 gap-4 text-xl font-black shadow-2xl shadow-primary/30 transition-all active:scale-95" asChild>
                        <a href={`tel:${req.phone}`}><Phone className="h-6 w-6" /> {lang === 'bn' ? 'যোগাযোগ করুন' : 'Contact Now'}</a>
                      </Button>
      ...
      <section className="py-24 bg-accent/10">
        <div className="container mx-auto px-4">
          <div className="text-center mb-20 space-y-4">
            <Badge className="bg-primary text-white border-none px-6 py-1.5 rounded-full text-sm font-black shadow-lg">{lang === 'bn' ? 'রক্তদান প্রক্রিয়া' : 'Donation Process'}</Badge>
            <h2 className="text-4xl md:text-6xl font-black font-headline">{lang === 'bn' ? 'মাত্র ৩টি সহজ ধাপে' : 'In Just 3 Simple Steps'}</h2>
            <div className="h-2 w-24 bg-primary mx-auto rounded-full mt-6 shadow-sm"></div>
          </div>
          <div className="grid md:grid-cols-3 gap-12 max-w-6xl mx-auto">
            {[
              { id: "1", title: t.home.processStep1, desc: t.home.processStep1Desc, icon: UserPlus },
              { id: "2", title: t.home.processStep2, desc: t.home.processStep2Desc, icon: Search },
              { id: "3", title: t.home.processStep3, desc: t.home.processStep3Desc, icon: HeartPulse }
            ].map((step, i) => (
          ...
          <div className="grid lg:grid-cols-2 gap-20 items-center max-w-7xl mx-auto">
            <div className="relative h-[500px] lg:h-[700px] rounded-[4rem] overflow-hidden shadow-[0_40px_100px_-20px_rgba(211,29,42,0.4)] border-8 border-accent">
              ...
              <div className="absolute bottom-10 left-10 right-10 bg-white/90 backdrop-blur-md p-8 rounded-3xl shadow-2xl border-l-8 border-primary">
                <p className="text-3xl font-black text-primary leading-tight">{lang === 'bn' ? '১ ব্যাগ রক্ত' : '1 Bag of Blood'} <br /><span className="text-foreground">{lang === 'bn' ? '৩ জনের প্রাণ বাঁচায়!' : 'Saves 3 Lives!'}</span></p>
              </div>
            </div>
            <div className="space-y-10">
              <div className="space-y-6">
                <Badge className="bg-primary/10 text-primary border-none px-6 py-2 rounded-full text-sm font-black">{lang === 'bn' ? 'কেন রক্ত দেবেন?' : 'Why Donate?'}</Badge>
                <h2 className="text-5xl md:text-7xl font-black font-headline leading-tight">{lang === 'bn' ? 'রক্তদানের' : 'Benefits of'} <br /><span className="text-primary">{lang === 'bn' ? 'উপকারিতা' : 'Donation'}</span></h2>
              </div>
              <div className="grid gap-8">
                {[
                  { title: lang === 'bn' ? "হার্টের স্বাস্থ্য ভালো রাখে" : "Heart Health", desc: lang === 'bn' ? "রক্তদান করলে শরীরে আয়রনের ভারসাম্য বজায় থাকে, যা হৃদরোগের ঝুঁকি কমায়।" : "Maintains iron balance, reducing heart risk.", icon: Heart },
                  { title: lang === 'bn' ? "নতুন রক্তকণিকা তৈরি" : "New Blood Cells", desc: lang === 'bn' ? "রক্ত দেওয়ার পর শরীর নতুন রক্তকণিকা তৈরি করে, যা আপনাকে আরও সতেজ রাখে।" : "Stimulates production of new red blood cells.", icon: Zap },
                  { title: lang === 'bn' ? "বিনামূল্যে স্বাস্থ্য পরীক্ষা" : "Free Checkup", desc: lang === 'bn' ? "রক্তদানের সময় আপনার হিমোগ্লোবিন, রক্তচাপ ও অন্যান্য পরীক্ষা বিনামূল্যে করা হয়।" : "Checks hemoglobin and BP for free.", icon: ShieldCheck },
                  { title: lang === 'bn' ? "মানসিক প্রশান্তি" : "Mental Peace", desc: lang === 'bn' ? "কারো জীবন বাঁচানোর চেয়ে বড় মানসিক তৃপ্তি আর কিছু হতে পারে না।" : "The joy of saving a life is unparalleled.", icon: Award }
                ].map((item, i) => (
              ...
              <Button size="lg" className="rounded-full px-16 h-16 text-2xl font-black bg-primary shadow-[0_20px_50px_rgba(211,29,42,0.4)] transition-all hover:scale-105 active:scale-95" asChild>
                <NextLink href="/register">{t.nav.register}</NextLink>
              </Button>
            </div>
          </div>
        </div>
            ...
            <div className="space-y-12 text-center md:text-left">
              ...
              <Button size="lg" className="bg-primary hover:bg-secondary h-20 px-16 rounded-full text-3xl font-black gap-6 group shadow-[0_30px_60px_rgba(211,29,42,0.4)] transition-all active:scale-95 border-4 border-white/10" asChild>
                <NextLink href="/eligibility">
                  {lang === 'bn' ? 'যোগ্যতা যাচাই করুন' : 'Check Eligibility'} <ArrowRight className="h-9 w-9 group-hover:translate-x-3 transition-transform" />
                </NextLink>
              </Button>
            </div>
      ...
      <section className="py-24 bg-white">
        <div className="container mx-auto px-4 max-w-6xl">
          <div className="text-center mb-20 space-y-6">
            <Badge className="bg-blue-600 text-white border-none px-8 py-2 rounded-full text-sm font-black shadow-md">{lang === 'bn' ? 'রক্তের গ্রুপের সামঞ্জস্যতা' : 'Blood Compatibility'}</Badge>
            <h2 className="text-4xl md:text-6xl font-black font-headline">{lang === 'bn' ? 'কে কাকে রক্ত দিতে পারবে?' : 'Who Can Donate To Whom?'}</h2>
            <p className="text-2xl text-muted-foreground font-bold">{lang === 'bn' ? 'নিচের চার্টটি দেখে আপনার প্রয়োজনীয় গ্রুপটি নিশ্চিত করুন।' : 'Check the chart below to find compatible groups.'}</p>
          </div>
            ...
            <Table>
              <TableHeader className="bg-primary text-white">
                <TableRow className="hover:bg-primary border-none">
                  <TableHead className="text-white font-black h-20 text-2xl px-10 text-center">{lang === 'bn' ? 'রক্তের গ্রুপ' : 'Blood Group'}</TableHead>
                  <TableHead className="text-white font-black h-20 text-2xl px-10 text-center">{lang === 'bn' ? 'রক্ত দিতে পারবেন' : 'Can Donate To'}</TableHead>
                  <TableHead className="text-white font-black h-20 text-2xl px-10 text-center">{lang === 'bn' ? 'রক্ত নিতে পারবেন' : 'Can Receive From'}</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
      ...
      <section className="py-24 bg-slate-950 text-white text-center relative border-t-8 border-primary">
        <div className="container mx-auto px-4 space-y-12">
          ...
          <div className="max-w-4xl mx-auto space-y-6">
            <h2 className="text-4xl md:text-6xl font-black font-headline">{lang === 'bn' ? 'জরুরি কোনো সাহায্য প্রয়োজন?' : 'Need Emergency Help?'}</h2>
            <p className="text-2xl text-slate-400 font-bold">{lang === 'bn' ? 'আমাদের হেল্পলাইন নম্বরে কল করুন সরাসরি সাহায্যের জন্য।' : 'Call our helpline directly for assistance.'}</p>
          ...
          <div className="pt-10 flex justify-center gap-6">
            <Button variant="outline" size="lg" className="rounded-full bg-white/5 border-2 border-white/10 hover:bg-primary hover:border-primary text-xl font-bold transition-all px-10" asChild>
              <a href="https://www.facebook.com/Roktooo" target="_blank" rel="noopener noreferrer">
                <MessageSquare className="mr-3 h-6 w-6" /> {lang === 'bn' ? 'মেসেজ দিন' : 'Send Message'}
              </a>
            </Button>
          </div>
        </div>
